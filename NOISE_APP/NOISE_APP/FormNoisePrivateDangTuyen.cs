...
namespace NOISE_APP
{
    public partial class FormNoisePrivateDangTuyen : Form
    {
        ...
        public FormNoisePrivateDangTuyen()
        {
            InitializeComponent();
        }
        ...
        public void TinhToan(DateTime dateCal)
        {
            ...
            var data = GetDataFromSever(dateCal);
                ...
                var mxdResult = System.IO.Path.Combine(btnFolder.EditValue.ToString(), string.Format("{0}.mxd", btnTemp.Text + dateCal.ToString("yyyyMMdd") + "_" + (dateCal.Hour).ToString()));
            ...
            try
            {
                    ...
                    && !string.IsNullOrEmpty(btnGDB.EditValue.ToString()))
                {
                    ...
                    for (int i = 0; i <= 23; i++)
                    {
                        ...
                        var mxdResult = System.IO.Path.Combine(btnFolder.EditValue.ToString(), string.Format("{0}.mxd", btnTemp.Text + dateTinhToan.ToString("yyyyMMdd") + "_" + (dateTinhToan.Hour).ToString()));
